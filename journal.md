...
## 12/04/26 Giovanni
Continuando a inputtare snana... Il modo di nascondere i parametri delle simulazioni al fitter è aggiungere +8 al parametro `FORMAT_MASK` di `snlc_sim.exe`. Nell'input file del fitter (`snfit_SDSS_custom.input`) c'è un parametro chiamato `OPT_REFORMAT_TEXT` impostato a 256. In realtà prima era rotto e impostato a `OPT_REFORMAT_TEXT += 256`, non essendo prima definito (suppongo) si rompeva completamente il programma e non partiva niente. Immagino sia stato impostato perché anche lì il manuale (12.4.2) dice che nasconde cose, ma da quello che ho capito serve solo a riformattare files. Non ho ancora compreso a pieno il senso di questo parametro e cosa faccia davvero, ma messo a 256 e facendo runnare `snlc_fit.exe` non sembra nemmeno outputtare il fit, ma riformattare appunto la simulazione in formato TEXT (file `.DAT`), o FITS se si usa invece `OPT_REFORMAT_FITS`. Fa tutto molto ridere perché l'output su terminale sembra fare un fit, ma l'output vero è poi una riformattazione dell'input?! È un po' tutto rotto e difficile da capire, ma non penso ci servirà comunque. La mia wild guess è che `OPT_REFORMAT_*` sia in realtà un parametro di `snana.exe` che in qualche modo viene richiamato da `snlc_fit.exe` e quindi `snana.exe` va il suo sporco lavoro di riformattare l'input e poi si ammazza. Questo comunque dovrebbe aver risolto il mistero del 19 febbraio...

A pagina 20 dell'articolo di riferimento invece spiega quella cosa di marginalizzare anche su H0, che potrebbe essere simpatico...
